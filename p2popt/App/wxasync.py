...
class WxAsyncApp(wx.App):
    ...
    def StartCoroutine(self, coroutine, obj,id = 0):
    ...
    def OnTaskCompleted(self, task):
        try:
            ...
            pass
        self.RunningTasks[task.id].remove(task)

    def OnDestroy(self, event, obj):
        # Cancel async callbacks
        for id in self.RunningTasks:
            task = self.RunningTasks[id]
            if task.obj == obj:
                if not task.done():
                    task.cancel()
...
def StartCoroutine(coroutine, obj,id =0):
    app = wx.App.Get()
    if not isinstance(app, WxAsyncApp):
        raise Exception("Create a 'WxAsyncApp' first")
