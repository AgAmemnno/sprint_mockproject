...
void Signal(inout Deal d,inout AMA ama,in MFI_Cros mfic){
     ...
     if(Calc && VIS_MODE==5){
        vis[IDX].mfi[YZID]   =  d.Balance;
        vis[IDX].mfi2[YZID]  =  d.Prof;
        vis[IDX].asset[YZID] =  npm;
     }
...
void main() {
    ...
    if(Calc && VIS_MODE==2){
         vis[IDX].asset[YZID]  =  rates[SIDX].tval;
    }else if(Calc && VIS_MODE==5){
        vis[IDX].mfi[YZID]  =  D.Balance;
        vis[IDX].mfi2[YZID] =  D.Prof;
        vis[IDX].asset[YZID] =  0;
    }
